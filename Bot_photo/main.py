...
async def textnew(message: types.Message):
    ...
    while count < 1:
        ...
        if str(message_arr[0]) == 'Экспорт':
            ...
            font = ImageFont.truetype("articulat-cf-extra-light.ttf", size=38)
    ...
    if str(message_arr[0]) == 'Дашборд':
        base = sqlite3.connect('PhotoDashbord.db')
        cur = base.cursor()
        image = Image.open("ДашбордФон.png")
        ...
        # Блок со спидометром
        image_strelka = Image.open("Стрелка.png")
        idraw = ImageDraw.Draw(image)
        MAX_SIZE = (220, 220)
        ...
        # Дата
        font = ImageFont.truetype("articulat-cf-extra-light.ttf", size=30)
        ...
        # Доллар
        font = ImageFont.truetype("articulat-cf-demi-bold.ttf", size=28)
        text = message_arr[2].replace('.', ',')
        idraw.text((166, 426), text, font=font, fill='#ffffff')
        if float(info_dollar[0][0]) == float(message_arr[2]):
            font = ImageFont.truetype("articulat-cf-demi-bold.ttf", size=26)
        ...
        # Пшеница
        font = ImageFont.truetype("articulat-cf-demi-bold.ttf", size=22)
        text = message_arr[5].replace('.', ',')
        idraw.text((64, 815), text + '$', font=font, fill='#ffffff')
        if float(info_psheno[0][0]) == float(message_arr[5]):
            font = ImageFont.truetype("articulat-cf-demi-bold.ttf", size=19)
        ...
        font = ImageFont.truetype("ArgentCFSuper.ttf", size=21)
        ...
        font = ImageFont.truetype("articulat-cf-demi-bold.ttf", size=23)
    ...
    if str(message_arr[0]) == 'Карта':
        base = sqlite3.connect('PhotoKarta.db')
        cur = base.cursor()
    ...
    if str(message_arr[0]) == 'Ексель':
        if str(message_arr[1]) == 'Рынок':
            ...
            conn = sqlite3.connect(r'C:\NovikovAA_WEB_Project\ProjectsPython\Bot_photo\PhotoRinok.db')
            df = pd.read_sql('select * from timedb', conn)
            df.to_excel(r'C:\NovikovAA_WEB_Project\ProjectsPython\Bot_photo\Data.xlsx', index=False)
            ...
            plt.savefig('plotCFO.png')
            image = open('plotCFO.png', 'rb')
            ...
            plt.savefig('plotUFO.png')
            image = open('plotUFO.png', 'rb')
            await bot.send_photo(id, image)

            table_info = open(r'C:\NovikovAA_WEB_Project\ProjectsPython\Bot_photo\Data.xlsx', 'rb')
        ...
        if str(message_arr[1]) == 'Экспорт':
            ...
            conn = sqlite3.connect(r'C:\NovikovAA_WEB_Project\ProjectsPython\Bot_photo\PhotoEksport.db')
            df = pd.read_sql('select * from timedb', conn)
            df.to_excel(r'C:\NovikovAA_WEB_Project\ProjectsPython\Bot_photo\Data.xlsx', index=False)
            ...
            plt.savefig('plotNovoros.png')
            image = open('plotNovoros.png', 'rb')
            await bot.send_photo(id, image)
            ...
            plt.savefig('plotTaman.png')
            image = open('plotTaman.png', 'rb')
            await bot.send_photo(id, image)
            ...
            plt.savefig('plotAzov.png')
            image = open('plotAzov.png', 'rb')
            await bot.send_photo(id, image)
            ...
            plt.savefig('plotAstraxan.png')
            image = open('plotAstraxan.png', 'rb')
            await bot.send_photo(id, image)
